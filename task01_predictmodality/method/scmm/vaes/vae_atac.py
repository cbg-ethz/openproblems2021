...
class Enc(nn.Module):
    def __init__(
        self, data_dim, latent_dim, num_hidden_layers, hidden_dim
    ):  # added hidden_dim
...
class Dec(nn.Module):
    ...
    def forward(self, z):
        d = self.dec(z)
        log_r = self.fc31(d).clamp(
            -12, 12
        )  # restrict to avoid torch.exp() over/underflow
        r = torch.exp(log_r)
        p = self.fc32(d)
        p = torch.sigmoid(p).clamp(
            Constants.eps, 1 - Constants.eps
        )  # restrict to avoid probs = 0,1
        g = self.fc33(d)
        g = torch.sigmoid(g)
        return r, p, g
...
class ATAC(VAE):
    ...
    def getDataLoaders(dataset, batch_size, shuffle=True, device="cuda"):
        ...
        dataloader = DataLoader(
            dataset, batch_size=batch_size, shuffle=shuffle, drop_last=False, **kwargs
        )
        return dataloader
